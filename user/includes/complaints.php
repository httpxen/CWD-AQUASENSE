<?php
// includes/complaints.php (Full Updated File with New Clock SVG Icon and Category Badge)
...
<div class="space-y-4">
    ...
    <?php else: ?>
        <?php while ($row = mysqli_fetch_assoc($list_res)): ?>
            ...
            $assigned_class = !empty($row['staff_name']) ? 'text-gray-800 font-medium' : 'text-gray-400 italic';
            ...
            // Prepare assigned display (adding badge next to name/role)
            $assigned_display = '';
            if (!empty($row['staff_name'])) {
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="status-badge ' . $assigned_badge . ' text-xs px-1 py-0.5">Assigned Staff</span>
                    </div>
                </div>';
            } else {
                $assigned_display = '
            ...
            $due_display = '';
            if ($row['action_due']): 
                $current_date = date('Y-m-d'); // Current date logic (adjust if needed for timezone)
                $due_date = $row['action_due'];
                $days_until_due = (strtotime($due_date) - strtotime($current_date)) / (60 * 60 * 24);
                
                $due_class = 'bg-green-50 text-green-700 border border-green-200'; // Default: Green for future
                if ($days_until_due <= 0) {
                    $due_class = 'bg-red-50 text-red-700 border border-red-200 animate-pulse'; // Red + pulse if overdue
                } elseif ($days_until_due <= 3) {
                    $due_class = 'bg-yellow-50 text-yellow-700 border border-yellow-200'; // Yellow if near (within 3 days)
                }
                $due_display = '
            ...
            <div class="complaint-card">
                <div class="complaint-header">
                    ...
                    <div class="text-right">
                        <?php echo $due_display; ?> <!-- Dynamic Due Date Badge Here -->
