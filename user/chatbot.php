...
<body class="bg-gray-50">
                    ...
                    <header class="chat-header relative z-10">
                        <div class="flex items-center gap-3">
                            <div class="header-icon">
                                <video autoplay loop muted playsinline class="w-9 h-9 object-contain rounded-full">
                                    <source src="../assets/icons/kuya-daloy.mp4" type="video/mp4">
                                    Your browser does not support the video tag.
                                </video>
                            </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Initial quick reply buttons for common actions
            function addInitialQuickReplies() {
                const initialReplies = document.createElement("div");
                initialReplies.className = 'flex flex-wrap gap-2 mt-4 p-4 bg-gray-50 rounded-lg';
                initialReplies.innerHTML = `
                    <button class="px-4 py-2 bg-blue-100 text-blue-700 rounded-full text-sm hover:bg-blue-200 transition-colors" onclick="sendUserMessage('I want to file a complaint')">File Complaint</button>
                    <button class="px-4 py-2 bg-green-100 text-green-700 rounded-full text-sm hover:bg-green-200 transition-colors" onclick="sendUserMessage('Check my bill')">Check Bill</button>
                    <button class="px-4 py-2 bg-purple-100 text-purple-700 rounded-full text-sm hover:bg-purple-200 transition-colors" onclick="sendUserMessage('Payment options')">Payment Options</button>
                    <button class="px-4 py-2 bg-yellow-100 text-yellow-700 rounded-full text-sm hover:bg-yellow-200 transition-colors" onclick="sendUserMessage('Usage tips')">Usage Tips</button>
                `;
                chatMessages.appendChild(initialReplies);
            }

            // Add initial message and quick replies
            const initialMsg = document.getElementById('initialMessage');
