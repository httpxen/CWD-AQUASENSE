...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($check_result && mysqli_num_rows($check_result) == 1) {
            ...
            if ($emailService->sendUsernameRecovery($email, $user['username'])) {
                $success = "Username recovery instructions have been sent to your email address. Please check your inbox (and spam folder).";
            } else {
                $error = "Failed to send recovery email. Please try again later.";
            }
        } else {
            // Email doesn't exist - show generic message for security
            $success = "If an account with that email exists, username recovery instructions have been sent. Please check your inbox (and spam folder).";
        }
    }
}
?>
...
<head>
  ...
  <title>Forgot Username | CWD AquaSense</title>
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md relative z-10 fade-in-up">
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-gray-500 mt-2 max-w-sm mx-auto">
          Enter your email address and we'll send you your registered username.
        </p>
      </div>
            ...
            <div>
              ...
              >
                <span class="absolute left-4">
                  <i class="fas fa-user text-blue-100 group-hover:text-white transition-colors text-sm"></i>
                </span>
                <span class="relative">Send Username</span>
              </button>
            </div>
          </form>
        <?php endif; ?>
