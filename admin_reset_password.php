...
// Updated: December 2025 - Added error handling for DB operations
...
if (empty($token)) {
    ...
    $sql = "SELECT email, reset_token, reset_expiry FROM staff WHERE reset_token = ? LIMIT 1";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $showForm) {
    ...
    if (empty($errors)) {
        ...
        $update_sql = "UPDATE staff SET password = ?, reset_token = NULL, reset_expiry = NULL WHERE reset_token = ?";
