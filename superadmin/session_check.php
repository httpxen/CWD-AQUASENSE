<?php
session_name('AdminSession'); // Shared session for admins (including SuperAdmin)
session_start();
...
if (!isset($_SESSION['staff_email']) || empty($_SESSION['staff_role'])) {
    header("Location: ../admin_login.php?message=Please log in as SuperAdmin.");
    exit();
}

// Fetch staff_id and update last_login in DB (for online status tracking)
...
// Role check (e.g., only SuperAdmin can access full dashboard)
if ($_SESSION['staff_role'] != 'SuperAdmin') {
    header("Location: ../admin_login.php?message=Access denied. SuperAdmin role required.");
    exit();
}
?>