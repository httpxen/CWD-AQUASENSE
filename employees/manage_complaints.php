...
// Build WHERE clause for filters (excluding staff_id - handled in JOIN)
$clauses = [];
$params = [];
$types = '';
...
// Count total complaints assigned to this staff
$count_sql = "
    SELECT COUNT(*) AS cnt 
    FROM complaints c 
    INNER JOIN (
...
// Fetch complaints list (only latest assignment for each complaint)
$list_sql = "
    ...
    LIMIT ? OFFSET ?
";
$types_paged = "i" . $types . "ii"; // staff_id + filters + limit + offset
...
<head>
    ...
    <title>My Complaints | CWD AquaSense</title>
    ...
    <style>
        ...
        /* Due Date Badge */
        .due-badge { border-radius: 0.5rem; padding: 0.25rem 0.5rem; font-size: 0.75rem; font-weight: 600; border-width: 1px; }
        /* Resolved Badge */
        .resolved-badge {
...
<body class="bg-gray-50">
                ...
                <div class="p-4 border-t border-gray-100">
                    <div class="flex items-center space-x-3 mb-4">
                        <div class="relative avatar-glow">
                            <img src="<?php echo e(get_avatar_src($staff['profile_picture'], $staff['name'] ?? 'Staff')); ?>" alt="Avatar" class="w-10 h-10 rounded-full object-cover"/>
                            <div class="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-green-500 border-2 border-white rounded-full animate-gentle-pulse"></div>
                        </div>
                        <div>
                            <p class="font-semibold text-gray-900"><?php echo e($staff['name'] ?? 'Support Staff'); ?></p>
                            <p class="text-xs text-gray-500"><?php echo e($staff['role'] ?? 'Support'); ?></p>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="flex items-center space-x-3 p-2 profile-card hover:bg-gray-50 rounded-xl transition-all duration-200 group cursor-pointer relative" id="profileDropdown">
                                ...
                                <div class="hidden md:block">
                                    <p class="text-sm font-semibold text-gray-900 truncate max-w-32"><?php echo e($staff['name'] ?? 'Support Staff'); ?></p>
                                    <p class="text-xs text-gray-500 truncate max-w-32"><?php echo e($staff['role'] ?? 'Support'); ?></p>
                                </div>
            ...
            <main class="p-6 space-y-6">
                ...
                <div class="card p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                        ...
                        <button onclick="applyFilters()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">Apply Filters</button>
                    </div>
                </div>
                ...
                <div class="card overflow-hidden">
                    <div class="flex items-center justify-between p-6 border-b border-gray-200">
                        <h2 class="text-lg font-semibold text-gray-900">My Assigned Complaints (<?php echo (int)$total_rows; ?>)</h2>
                        <p class="text-xs text-gray-500">Showing <?php echo min($per_page, $total_rows - $offset); ?> of <?php echo $total_rows; ?> results</p>
                    </div>
                    <div class="p-6 space-y-4">
                        <?php if ($total_rows === 0): ?>
                            <div class="text-center py-12 text-gray-500">No complaints assigned to you yet. Check back later for new assignments.</div>
                        <?php else: ?>
                            <?php while ($row = mysqli_fetch_assoc($list_res)): ?>
                                ...
                                if ($row['action_due']): 
                                    ...
                                    $due_text = 'Due: ' . date('M d, Y', strtotime($due_date));
                                    if ($days_until_due <= 0) {
                                        $due_class = 'bg-red-50 text-red-700 border border-red-200 animate-pulse'; // Red + pulse if overdue
                                        $due_text = 'Overdue: ' . date('M d, Y', strtotime($due_date));
                                    } elseif ($days_until_due <= 3) {
                                        $due_class = 'bg-yellow-50 text-yellow-700 border border-yellow-200'; // Yellow if near
                                        $due_text = 'Due soon: ' . date('M d, Y', strtotime($due_date));
                                    }
                                    $due_display = '
                                    <span class="due-badge inline-block ' . $due_class . ' px-2 py-1 text-xs font-medium flex items-center">
    ...
    <!-- Status Update Modal (for Support Staff - limited to Pending/In Progress) -->
    <div id="statusModal" class="modal">
        <div class="bg-white w-11/12 max-w-md rounded-2xl p-6 shadow-2xl">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-900">Update Complaint Status</h3>
            ...
            <form id="statusForm" method="POST" action="update_status.php">
                ...
                <div class="space-y-4">
                    ...
                    <div id="resolvedDateField" style="display: none;">
                        ...
                        <input type="datetime-local" name="resolved_at" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" step="1">
                    </div>
                    <div>
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <button type="submit" class="btn-primary px-4 py-2 rounded-lg text-sm font-medium">Update Status</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function openStatusModal(id) {
            ...
            // Auto-focus the status select
            document.querySelector('#statusModal select[name="status"]').focus();
        }
        ...
        function toggleResolvedDate(select) {
            ...
            if (select.value === 'Resolved' || select.value === 'Closed') {
                ...
                // Set default resolved date to now if empty
                if (!resolvedDateField.querySelector('input').value) {
                    const now = new Date();
                    ...
                    resolvedDateField.querySelector('input').value = `${year}-${month}-${day}T${hours}:${minutes}:${seconds}`;
                }
            } else {
                resolvedDateField.style.display = 'none';
                resolvedDateField.querySelector('input').value = '';
            }
        }
        ...
        // Prevent form submission on Enter in filters (use Apply Filters button instead)
